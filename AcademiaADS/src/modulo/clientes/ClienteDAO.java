...
public class ClienteDAO {
    ...
    public int Insert(Cliente objCli) throws Exception {
        ...
        String SQL = "insert into "+dados.getBD()+"."+dados.getSCHE()+".cliente(nome_cli, end_cli, telefone_cli, date_registro) values (?,?,?,?)";
    ...
    public boolean Update(Cliente objCli) throws Exception {
        con = new Conexao();
        String SQL = "UPDATE "+dados.getBD()+"."+dados.getSCHE()+".cliente set nome_cli = ?, end_cli = ?, telefone_cli = ? WHERE id_cli = ?";
    ...
    public boolean Delete(int id) throws Exception {
        con = new Conexao();
        String SQL = "DELETE from "+dados.getBD()+"."+dados.getSCHE()+".cliente where id_cli = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
        ps.setInt(1, id);
        boolean p = ps.executeUpdate() > 0;
        con.getCONEXAO().close();
    ...
    public List<Cliente> TabelaPesquisa() throws Exception {
        ...
        String SQL = "select * from "+dados.getBD()+"."+dados.getSCHE()+".cliente order by 3";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
    ...
    public List<Cliente> PesquisaNome(String nome) throws Exception {
        ...
        Cliente objCli = null;
        List<Cliente> cliente = new ArrayList<>();
        String SQL = "select * from "+dados.getBD()+"."+dados.getSCHE()+".cliente where nome_cli like ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
