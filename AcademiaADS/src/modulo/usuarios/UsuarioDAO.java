...
public class UsuarioDAO {
    ...
    private DadosDataBase dados = new DadosDataBase();
    
    public boolean Insert(Usuario objFun) throws Exception {
        
        con = new Conexao();
        String SQL = "insert into "+dados.getBD()+"."+dados.getSCHE()+".usuario(matricula, nome, email, date_nas, telefone, funcao, senha) values (?,?,?,?,?,?,?)";
    ...
    public Usuario PesquisaPorMatricula(int matricula) throws Exception {
        ...
        String SQL = "select * from "+dados.getBD()+"."+dados.getSCHE()+".usuario where matricula = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
    ...
    public boolean Update(Usuario objFun) throws Exception {
        con = new Conexao();
        String SQL = "UPDATE "+dados.getBD()+"."+dados.getSCHE()+".usuario set nome = ?, email = ?, date_nas = ?, telefone = ?, funcao = ? WHERE matricula = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
    ...
    public boolean UpdateSenha(Usuario objFun) throws Exception {
        con = new Conexao();
        String SQL = "UPDATE "+dados.getBD()+"."+dados.getSCHE()+".usuario set senha = ? WHERE matricula = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
    ...
    public boolean Delete(int matricula) throws Exception {
        con = new Conexao();
        String SQL = "DELETE from "+dados.getBD()+"."+dados.getSCHE()+".usuario where matricula = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
        ps.setInt(1, matricula);
        boolean p = ps.executeUpdate() > 0;
        con.getCONEXAO().close();
    ...
    public List<Usuario> TabelaPesquisa() throws Exception {
        ...
        String SQL = "select * from "+dados.getBD()+"."+dados.getSCHE()+".usuario order by 3";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            objFun = new Usuario();
    ...
    public Usuario PesquisaID(int id) throws Exception {
        con = new Conexao();
        Usuario objFun = null;
        String SQL = "select * from "+dados.getBD()+"."+dados.getSCHE()+".usuario where idusuario = ?";
        PreparedStatement ps = con.getCONEXAO().prepareStatement(SQL);
        ps.setInt(1, id);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
